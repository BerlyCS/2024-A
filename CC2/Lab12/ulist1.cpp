...
public:
    ...
    void insert_ord(T x, T y) {
        //CASO 1: head no existe
        if (head == nullptr) {
            head = make_unique<coord<T>>(x, y);
            last = head.get();
            ptr = head.get();
            return;
        //CASO 2: esta en la ultima posicion
        } else if (last != nullptr && last->y < y) {
            last->next = make_unique<coord<T>>(x, y);
            last = last->next.get();
            return;
        //CASO 3: insertar en la primera posicion
        } else if (y < head->y) {
            auto tmp = make_unique<coord<T>>(x, y);
            tmp->next = std::move(head);
            head = std::move(tmp);
            return;
        // CASO 4 : se itera en toda la lista
        } else {
            ptr = head.get();
